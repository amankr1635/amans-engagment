  ...
  return (
    <section ref={ref} className="section-padding cinematic-gradient text-center relative overflow-hidden">
      ...
      <div className={`relative z-10 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className="flex justify-center gap-3 sm:gap-4 md:gap-8 lg:gap-12 mt-12 perspective-container px-4">
          {units.map((u, i) => (
            <div
              key={u.label}
              className={`flex flex-col items-center transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
              style={{ transitionDelay: `${i * 150 + 300}ms` }}
            >
              <div className="card-3d w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 rounded-lg flex items-center justify-center border border-gold/20 bg-card/50"
                style={{
                  boxShadow: "0 0 30px hsla(40, 70%, 50%, 0.1), inset 0 1px 0 hsla(40, 70%, 50%, 0.1)"
                }}
              >
                <span className="font-heading text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-gold-gradient">
                  {String(u.value).padStart(2, "0")}
                </span>
              </div>
              <span className="text-muted-foreground text-[0.65rem] sm:text-xs md:text-sm mt-2 md:mt-3 font-sans tracking-widest uppercase">{u.label}</span>
            </div>
          ))}
        </div>
      </div>
